...
public class RequestManagerRetriever implements Handler.Callback {
    ...
    private static FrameWaiter buildFrameWaiter(GlideExperiments experiments) {
        ...
        return experiments.isEnabled(WaitForFramesAfterTrimMemory.class) ? new FirstFrameAndAfterTrimMemoryWaiter() : new FirstFrameWaiter();
    }
    ...
    public RequestManager get(@NonNull Context context) {
        if (context == null) {
            ...
            } else if (// Only unwrap a ContextWrapper if the baseContext has a non-null application context.
            // Context#createPackageContext may return a Context without an Application instance,
            // in which case a ContextWrapper may be used to attach one.
            context instanceof ContextWrapper && ((ContextWrapper) context).getBaseContext().getApplicationContext() != null) {
    ...
    public RequestManager get(@NonNull Activity activity) {
        if (Util.isOnBackgroundThread()) {
            ...
            return fragmentGet(activity, fm, /*parentHint=*/
            null, isActivityVisible(activity));
        }
    }
    ...
    RequestManagerFragment getRequestManagerFragment(Activity activity) {
        return getRequestManagerFragment(activity.getFragmentManager(), /*parentHint=*/
        null);
    }
    ...
    SupportRequestManagerFragment getSupportRequestManagerFragment(FragmentManager fragmentManager) {
        return getSupportRequestManagerFragment(fragmentManager, /*parentHint=*/
        null);
    }
    ...
    private boolean verifyOurFragmentWasAddedOrCantBeAdded(android.app.FragmentManager fm, boolean hasAttemptedToAddFragmentTwice) {
        ...
        android.app.FragmentTransaction transaction = fm.beginTransaction().add(newlyAddedRequestManagerFragment, FRAGMENT_TAG);
        ...
        handler.obtainMessage(ID_REMOVE_FRAGMENT_MANAGER, HAS_ATTEMPTED_TO_ADD_FRAGMENT_TWICE, /* arg2= */
        0, fm).sendToTarget();
    ...
    private boolean verifyOurSupportFragmentWasAddedOrCantBeAdded(FragmentManager supportFm, boolean hasAttemptedToAddFragmentTwice) {
        ...
        // Otherwise we should make another attempt to commit the fragment and loop back again in the
        // next frame to verify.
        FragmentTransaction transaction = supportFm.beginTransaction().add(newlyAddedSupportRequestManagerFragment, FRAGMENT_TAG);
        // If the Activity is re-created and a Glide request was started for that Activity prior to the
        // re-creation, then there will be an old RequestManagerFragment that is re-created as well.
        // Under normal circumstances we find and re-use that Fragment rather than creating a new one.
        // However, if the first Glide request for the re-created Activity occurs before the Activity is
        ...
        handler.obtainMessage(ID_REMOVE_SUPPORT_FRAGMENT_MANAGER, HAS_ATTEMPTED_TO_ADD_FRAGMENT_TWICE, /*arg2=*/
        0, supportFm).sendToTarget();
        if (Log.isLoggable(TAG, Log.DEBUG)) {
            Log.d(TAG, "We failed to add our Fragment the first time around, trying again...");
        }
        return false;
