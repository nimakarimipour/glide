...
class SourceGenerator implements DataFetcherGenerator, DataFetcherGenerator.FetcherReadyCallback {
    ...
    private boolean cacheData(Object dataToCache) throws IOException {
        ...
        try {
            ...
            if (diskCache.get(newOriginalKey) != null) {
                ...
                if (Log.isLoggable(TAG, Log.DEBUG)) {
                    Log.d(TAG, "Attempt to write: " + originalKey + ", data: " + dataToCache + " to the disk" + " cache failed, maybe the disk cache is disabled?" + " Trying to decode the data directly...");
                }
    ...
    @Override
    public void onDataFetcherFailed(@Nullable Key sourceKey, Exception e, DataFetcher<?> fetcher, DataSource dataSource) {
