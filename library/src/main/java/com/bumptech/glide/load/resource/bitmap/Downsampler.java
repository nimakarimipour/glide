...
public final class Downsampler {
    ...
    public Resource<Bitmap> decode(ByteBuffer buffer, int requestedWidth, int requestedHeight, Options options) throws IOException {
        return decode(new ImageReader.ByteBufferReader(buffer, parsers, byteArrayPool), requestedWidth, requestedHeight, options, EMPTY_CALLBACKS);
    }
    ...
    public Resource<Bitmap> decode(InputStream is, int requestedWidth, int requestedHeight, Options options, DecodeCallbacks callbacks) throws IOException {
        return decode(new ImageReader.InputStreamImageReader(is, parsers, byteArrayPool), requestedWidth, requestedHeight, options, callbacks);
    }

    @VisibleForTesting
    void decode(byte[] bytes, int requestedWidth, int requestedHeight, Options options) throws IOException {
        decode(new ImageReader.ByteArrayReader(bytes, parsers, byteArrayPool), requestedWidth, requestedHeight, options, EMPTY_CALLBACKS);
    }

    @VisibleForTesting
    void decode(File file, int requestedWidth, int requestedHeight, Options options) throws IOException {
        decode(new ImageReader.FileReader(file, parsers, byteArrayPool), requestedWidth, requestedHeight, options, EMPTY_CALLBACKS);
    }
    ...
    public Resource<Bitmap> decode(ParcelFileDescriptor parcelFileDescriptor, int outWidth, int outHeight, Options options) throws IOException {
        return decode(new ImageReader.ParcelFileDescriptorImageReader(parcelFileDescriptor, parsers, byteArrayPool), outWidth, outHeight, options, EMPTY_CALLBACKS);
    }
    ...
    @Nullable
    private Bitmap decodeFromWrappedStreams(ImageReader imageReader, BitmapFactory.Options options, @Nullable DownsampleStrategy downsampleStrategy, @Nullable DecodeFormat decodeFormat, @Nullable PreferredColorSpace preferredColorSpace, boolean isHardwareConfigAllowed, int requestedWidth, int requestedHeight, boolean fixBitmapToRequestedDimensions, DecodeCallbacks callbacks) throws IOException {
        ...
        int targetWidth = requestedWidth == Target.SIZE_ORIGINAL ? (isRotationRequired(degreesToRotate) ? sourceHeight : sourceWidth) : requestedWidth;
        int targetHeight = requestedHeight == Target.SIZE_ORIGINAL ? (isRotationRequired(degreesToRotate) ? sourceWidth : sourceHeight) : requestedHeight;
        ImageType imageType = imageReader.getImageType();
        calculateScaling(imageType, imageReader, callbacks, bitmapPool, downsampleStrategy, degreesToRotate, sourceWidth, sourceHeight, targetWidth, targetHeight, options);
    ...
    private static void calculateScaling(ImageType imageType, ImageReader imageReader, DecodeCallbacks decodeCallbacks, BitmapPool bitmapPool, @Nullable DownsampleStrategy downsampleStrategy, int degreesToRotate, int sourceWidth, int sourceHeight, int targetWidth, int targetHeight, BitmapFactory.Options options) throws IOException {
        ...
        SampleSizeRounding rounding = downsampleStrategy.getSampleSizeRounding(orientedSourceWidth, orientedSourceHeight, targetWidth, targetHeight);
        if (rounding == null) {
    ...
    private void calculateConfig(ImageReader imageReader, @Nullable DecodeFormat format, boolean isHardwareConfigAllowed, boolean isExifOrientationRequired, BitmapFactory.Options optionsWithScaling, int targetWidth, int targetHeight) {
    ...
    private static void logDecode(int sourceWidth, int sourceHeight, String outMimeType, BitmapFactory.Options options, @Nullable Bitmap result, int requestedWidth, int requestedHeight, long startTime) {
