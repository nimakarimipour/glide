...
class EngineJob<R> implements DecodeJob.Callback<R>, Poolable {
  ...
  @SuppressWarnings({
    ...
    void notifyCallbacksOfResult() {
        ...
        Resource<?> localResourceCopy;
        synchronized (this) {
            stateVerifier.throwIfRecycled();
            localResourceCopy = resource;
            if (isCancelled) {
                // Check if resource is not null before recycling
                if (localResourceCopy != null) {
                    localResourceCopy.recycle();
                }
                release();
                return;
            } else if (cbs.isEmpty()) {
            ...
            engineResource = engineResourceFactory.build(localResourceCopy, isCacheable, key, resourceListener);
            hasResource = true;
            copy = cbs.copy();
