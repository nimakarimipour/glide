...
public final class GranularRoundedCorners extends BitmapTransformation {
    ...
    protected Bitmap transform(@NonNull BitmapPool pool, @NonNull Bitmap toTransform, int outWidth, int outHeight) {
        return TransformationUtils.roundedCorners(pool, toTransform, topLeft, topRight, bottomRight, bottomLeft);
    }
    ...
    public void updateDiskCacheKey(@NonNull MessageDigest messageDigest) {
        ...
        byte[] radiusData = ByteBuffer.allocate(16).putFloat(topLeft).putFloat(topRight).putFloat(bottomRight).putFloat(bottomLeft).array();
