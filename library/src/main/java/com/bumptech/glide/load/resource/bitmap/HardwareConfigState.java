...
public final class HardwareConfigState {
     ...
     * details.
     */
    public static final boolean BLOCK_HARDWARE_BITMAPS_WHEN_GL_CONTEXT_MIGHT_NOT_BE_INITIALIZED = Build.VERSION.SDK_INT < Build.VERSION_CODES.Q;

    /**
     * Support for the hardware bitmap config was added in Android O.
     */
    public static final boolean HARDWARE_BITMAPS_SUPPORTED = Build.VERSION.SDK_INT >= Build.VERSION_CODES.O;
    ...
    private boolean areHardwareBitmapsBlockedByAppState() {
        return BLOCK_HARDWARE_BITMAPS_WHEN_GL_CONTEXT_MIGHT_NOT_BE_INITIALIZED && !isHardwareConfigAllowedByAppState.get();
    }
    ...
    private static boolean isHardwareConfigDisallowedByB112551574() {
        ...
        // This method will only be called once, so simple iteration is reasonable.
        for (// This is sadly a list of prefixes, not models. We no longer have the data that shows us
        ...
        // Moto
        // Moto
        String prefixOrModelName : Arrays.asList("SC-04J", "SM-N935", "SM-J720", "SM-G570F", "SM-G570M", "SM-G960", "SM-G965", "SM-G935", "SM-G930", "SM-A520", "SM-A720F", "moto e5", "moto e5 play", "moto e5 plus", "moto e5 cruise", "moto g(6) forge", "moto g(6) play")) {
    ...
    private int getMaxFdCount() {
        return manualOverrideMaxFdCount != NO_MAX_FD_COUNT ? manualOverrideMaxFdCount : sdkBasedMaxFdCount;
    }
    ...
    private synchronized boolean isFdSizeBelowHardwareLimit() {
        if (++decodesSinceLastFdCheck >= MINIMUM_DECODES_BETWEEN_FD_CHECKS) {
            ...
            if (!isFdSizeBelowHardwareLimit && Log.isLoggable(Downsampler.TAG, Log.WARN)) {
                Log.w(Downsampler.TAG, "Excluding HARDWARE bitmap config because we're over the file descriptor limit" + ", file descriptors " + currentFds + ", limit " + maxFdCount);
            }
        }
