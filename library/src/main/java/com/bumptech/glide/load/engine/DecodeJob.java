...
class DecodeJob<R> implements DataFetcherGenerator.FetcherReadyCallback, Runnable, Comparable<DecodeJob<?>>, Poolable {
    ...
    DecodeJob<R> init(GlideContext glideContext, @Nullable Object model, EngineKey loadKey, Key signature, int width, int height, Class<?> resourceClass, Class<R> transcodeClass, Priority priority, DiskCacheStrategy diskCacheStrategy, Map<Class<?>, Transformation<?>> transformations, boolean isTransformationRequired, boolean isScaleOnlyOrNoTransform, boolean onlyRetrieveFromCache, Options options, Callback<R> callback, int order) {
        decodeHelper.init(glideContext, model, signature, width, height, diskCacheStrategy, resourceClass, transcodeClass, priority, options, transformations, isTransformationRequired, isScaleOnlyOrNoTransform, diskCacheProvider);
    ...
    private void notifyComplete(Resource<R> resource, @Nullable DataSource dataSource, boolean isLoadedFromAlternateCacheKey) {
        setNotifiedOrThrow();
    ...
    @Override
    public void onDataFetcherReady(@Nullable Key sourceKey, @Nullable Object data, DataFetcher<?> fetcher, DataSource dataSource, @Nullable Key attemptedKey) {
    ...
    @Override
    public void onDataFetcherFailed(@Nullable Key attemptedKey, Exception e, DataFetcher<?> fetcher, DataSource dataSource) {
        fetcher.cleanup();
    ...
    private void notifyEncodeAndRelease(Resource<R> resource, @Nullable DataSource dataSource, boolean isLoadedFromAlternateCacheKey) {
    ...
    @Nullable
    private <Data> Resource<R> decodeFromData(@Nullable DataFetcher<?> fetcher, @Nullable Data data, @Nullable DataSource dataSource) throws GlideException {
        try {
            if (data == null) {
                return null;
            }
    ...
    private <Data> Resource<R> decodeFromFetcher(Data data, @Nullable DataSource dataSource) throws GlideException {
    ...
    private <Data, ResourceType> Resource<R> runLoadPath(Data data, @Nullable DataSource dataSource, @Nullable LoadPath<Data, ResourceType, R> path) throws GlideException {
    ...
    private void logWithTimeAndKey(String message, long startTime, @Nullable String extraArgs) {
        Log.v(TAG, message + " in " + LogTime.getElapsedMillis(startTime) + ", load key: " + loadKey + (extraArgs != null ? ", " + extraArgs : "") + ", thread: " + Thread.currentThread().getName());
    }
    ...
    interface Callback<R> {
        ...
        void onResourceReady(Resource<R> resource, @Nullable DataSource dataSource, boolean isLoadedFromAlternateCacheKey);
