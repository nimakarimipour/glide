...
class DecodeJob<R>
  ...
  private int getPriority() {
        if (priority == null) {
            throw new IllegalStateException("Priority is not initialized");
        }
  ...
  @Nullable private <Data> Resource<R> decodeFromData(
      ...
      try {
        ...
        Nullability.castToNonnull(fetcher, "mandatory parameter").cleanup();
      }
    }
  ...
  @Synthetic
        ...
        <Z> Resource<Z> onResourceDecoded(   @Nullable DataSource dataSource, @NonNull Resource<Z> decoded) {
          ...
          boolean isFromAlternateCacheKey = !decodeHelper.isSourceKey(Nullability.castToNonnull(currentSourceKey));
